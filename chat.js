...
const firebaseConfig = {
  ...
  storageBucket: "globalchat-2d669.firebasestorage.app", // Fixed storage bucket URL
  ...
  try {
    ...
    if (followSnapshot.empty) {
      ...
      await addDoc(collection(db, 'follows'), {
        ...
        timestamp: Date.now() // Changed to use numeric timestamp
      });
...
// Function to upload media files - modified to use uploadBytes
async function uploadMedia(file) {
  try {
    const fileRef = storageRef(storage, `imageSharing/${currentUser.uid}/${Date.now()}_${file.name}`);
...
async function sendMessage(parentMessageId = null) {
  ...
  try {
    ...
    const messageData = {
      ...
      text: messageText,
      media: mediaUrls,
      timestamp: currentTime, // Changed to numeric timestamp
